...
-   **Voice Messages:** Shared `useAudioRecorder` hook (`src/hooks/useAudioRecorder.ts`) used by ModerationAdmin, MentorResponsesPage, and LessonDetailPage. Features: timeslice recording (1s chunks), `recorder.onerror` + `track.onended` monitoring, 15-minute max duration with 1-min warning, retry on upload failure with auto-download fallback, size validation (10MB warning).
